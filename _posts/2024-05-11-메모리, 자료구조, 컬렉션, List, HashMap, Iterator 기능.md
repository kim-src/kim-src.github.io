---
title: 메모리, 자료구조, 컬렉션, List, Map, Set, Iterator 기능
date: 2024-05-11 18:00:00 +09:00
categories: [1. Fundamental, Java]
tags: [Java, Fundamental, Memory, Data Structure, Collections, Collections Framework, List, ArrayList, LinkedList, hMap, Set, Contains, Iterator, HashMap, Hash, Stack, Queue, Heap, Brute Force, Sorting]
---
...
> - 예를들어 Map의 경우 자료구조는 맞지만 Collections Framework에 포함되지는 않습니다.
> - 실무에서 자주 사용되는 자료구조는 리스트(List), 맵(Map), 셋(Set) 등이 있습니다.
...
### 🔔 맵(Map)
### 📌 Map 소개
> - Map은 데이터가 key 및 value 쌍으로 구성되는 HashTable 자료구조 형태로 구성됩니다.
> - 그래서 데이터 저장 또는 삭제 속도가 다른 자료구조에 비해 느린 편입니다.
> - 그런데 key라는 고유값으로 데이터를 탐색하기 때문에 탐색 속도는 매우 빠른 편입니다.
> - 따라서 프로젝트에서 데이터 탐색 기능이 빈번하게 요구되는 경우 Map을 이용합니다.

### 📌 Map 특징
> - Map을 사용하려면 제네릭에 해당 key 및 value의 데이터 유형을 함께 설정합니다.
> - 각각의 Map 객체는 주소 또는 값으로 구분되며 이때의 주소는 16진수의 HashCode입니다.
> - Map은 데이터의 순서가 없기 때문에 임의의 중간 삽입 또는 삭제가 불가능합니다.
> - 참고로 LinkedMap의 경우 데이터 순서는 있지만 인덱스가 없습니다.

### 📌 Map 활용
> - Map의 key 데이터는 중복을 허용하지 않습니다.
...
> - 예를들면 Map에는 containsKey 및 containsValue, List에는 contains가 있습니다.
...
> - Map의 경우 .keySet, .values 등을 통해 key 또는 value의 집합을 순회할 수 있습니다.
